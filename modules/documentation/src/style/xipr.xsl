...
<!-- XIPr is licensed under the GNU Lesser General Public License (LGPL).
See http://creativecommons.org/licenses/LGPL/2.1/ for licensing details. -->
<!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
<!-- XIPr Instructions: Include xipr.xsl in your XSLT 2.0 stylesheet using <xsl:include href=".../xipr.xsl">
and initiate the XInclude process at any node (only document and element nodes are reasonable node kinds, though) of a
 given XML document with the following instruction: <xsl:apply-templates select="$node" mode="xipr"/> -->
<!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                xmlns:xipr="http://dret.net/projects/xipr/"
                xmlns:dyn="urn:jetm/build/dynamic-values"

                >

  <!-- JETM extension in order to alter dynamic values -->
  <xsl:import href="dynamic-values.xsl" />

  <xsl:template match="dyn:pubdate" mode="xipr-internal">
    <xsl:value-of select="$release.date" />
  </xsl:template>

   <xsl:template match="dyn:releaseinfo" mode="xipr-internal">
  ...
  <xsl:template match="xi:include" mode="xipr-internal">
    ...
    <xsl:if test="count(xi:fallback) > 1 or exists(xi:include) or exists(xi:*[local-name() ne 'fallback'])">
      <xsl:sequence
        select="xipr:message('xi:include elements may only have no or one single xi:fallback element as their only xi:* child', 'fatal')"/>
    </xsl:if>
    ...
    <xsl:choose>
      <xsl:when test="@parse eq 'xml' or empty(@parse)">
        ...
        <xsl:if test="empty(@href | @xpointer)">
          ...
          <xsl:sequence
            select="xipr:message('For parse=&quot;xml&quot;, at least one the href or xpointer attributes must be present', 'fatal')"/>
        </xsl:if>
                ...
                <xsl:variable name="xpointer-node">
                  <xsl:choose>
                    ...
                    <xsl:when test="matches(@xpointer, '^[\i-[:]][\c-[:]]*$')">
                      <xsl:copy-of
                        select="xipr:include(id(@xpointer, $include-doc), $include-uri, @xpointer, $uri-history, $xpointer-history)"/>
                    </xsl:when>
                    ...
                    <xsl:when
                      test="matches(@xpointer, '^element\([\i-[:]][\c-[:]]*((/[1-9][0-9]*)+)?|(/[1-9][0-9]*)+\)$')">
                      ...
                      <xsl:choose>
                        <xsl:when test="not(contains($element-pointer, '/'))">
                          ...
                          <xsl:copy-of
                            select="xipr:include(id($element-pointer, $include-doc), $include-uri, @xpointer, $uri-history, $xpointer-history)"/>
                        </xsl:when>
                        <xsl:otherwise>
                          <!-- child sequence evaluation starts from the root or from an element identified by a NCName. -->
                          <xsl:copy-of
                            select="xipr:include(xipr:child-sequence( if ( starts-with($element-pointer, '/') ) then $include-doc else id(substring-before($element-pointer, '/'), $include-doc), substring-after($element-pointer, '/')), $include-uri, @xpointer, $uri-history, $xpointer-history)"/>
                        </xsl:otherwise>
                      </xsl:choose>
                    </xsl:when>
                    <xsl:otherwise>
                      ...
                      <xsl:sequence
                        select="xipr:message('XIPr only supports the XPointer element() scheme (skipping...)', 'warning')"/>
                    </xsl:otherwise>
                  </xsl:choose>
                </xsl:variable>
                <xsl:choose>
                  ...
                  <xsl:otherwise>
                    ...
                    <xsl:sequence
                      select="xipr:message(concat('Evaluation of xpointer ', @xpointer, ' returned nothing'), 'resource')"/>
      ...
      <xsl:when test="@parse eq 'text'">
        <xsl:if test="exists(@xpointer)">
          ...
          <xsl:sequence
            select="xipr:message('The xpointer attribute is not allowed for parse=&quot;text&quot;', 'warning')"/>
        </xsl:if>
        <xsl:choose>
          <xsl:when test="unparsed-text-available($include-uri)">
            <xsl:value-of
              select="if ( empty(@encoding) ) then unparsed-text($include-uri) else unparsed-text($include-uri, string(@encoding))"/>
          </xsl:when>
          <xsl:otherwise>
      ...
      <xsl:otherwise>
        ...
        <xsl:sequence
          select="xipr:message(concat('Unknown xi:include attribute value parse=&quot;', @parse ,'&quot;'), 'fatal')"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:function name="xipr:child-sequence">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:sequence
          select="xipr:child-sequence($context/*[number(substring-before($path, '/'))], substring-after($path, '/'))"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:function>
